    ...
    <script type="text/javascript">
        ...
        function createTableRow(rowId, tableId, data = null) {
            ...
            class="disabled:bg-slate-100 disabled:cursor-not-allowed dark:disabled:bg-darkmode-800/50 dark:disabled:border-transparent [&amp;[readonly]]:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50 [&amp;[readonly]]:dark:border-transparent transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80 [&amp;[type='file']]:border file:mr-4 file:py-2 file:px-4 file:rounded-l-md file:border-0 file:border-r-[1px] file:border-slate-100/10 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-500/70 hover:file:bg-200 group-[.form-inline]:flex-1 group-[.input-group]:rounded-none group-[.input-group]:[&amp;:not(:first-child)]:border-l-transparent group-[.input-group]:first:rounded-l group-[.input-group]:last:rounded-r group-[.input-group]:z-10 datepicker w-25">
                    ...
                    <td data-tw-merge class="px-5 py-3 border-b dark:border-darkmode-300 border-l border-r border-t"><div id="attachment_view_box-${claimType}-${rowNumber}" class="hidden relative h-28 w-28 col-span-3 cursor-pointer image-fit zoom-in md:col-span-2"><img src="" id="attachment_view-${claimType}-${rowNumber}" /><span data-placement="top" onclick="handleDeleteFile('${rowNumber}', '${claimType}', '${data?.expenseClaimDetail?.id || ''}')" class="tooltip cursor-pointer absolute top-0 right-0 w-5 h-5 -mt-2 -mr-2 bg-white rounded-full"><span class="flex items-center justify-center w-full h-full text-white border rounded-full border-danger/50 bg-danger/80"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="x" class="lucide lucide-x h-4 w-4 stroke-[1.3]"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg></span></span><input type="hidden" value="" name="attachment" id="attachment_url-${claimType}-${rowNumber}"></input></div><input id="attachment-${claimType}-${rowNumber}" name="attachment" onChange="handleUpload('${rowNumber}', '${claimType}', '${data?.expenseClaimDetail?.id || ''}')" data-tw-merge type="file" placeholder="Input file" class="disabled:bg-slate-100 disabled:cursor-not-allowed dark:disabled:bg-darkmode-800/50 dark:disabled:border-transparent [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 [&[readonly]]:dark:border-transparent transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80 [&[type='file']]:border file:mr-4 file:py-2 file:px-4 file:rounded-l-md file:border-0 file:border-r-[1px] file:border-slate-100/10 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-500/70 hover:file:bg-200 group-[.form-inline]:flex-1 group-[.input-group]:rounded-none group-[.input-group]:[&:not(:first-child)]:border-l-transparent group-[.input-group]:first:rounded-l group-[.input-group]:last:rounded-r group-[.input-group]:z-10"></td>
                    <td data-tw-merge class="px-5 py-3 border-b dark:border-darkmode-300 border-l border-r border-t">
        ...
        function handleUpload(rowId, type, selectedId = null, data = null) {
            ...
            $("#form-submit :input").each(function(e) {
                ...
                if (isEmpty) {
                    showErrorNotification('error', 'Please fill mandatory field');
                    e.preventDefault();
                } else {
                    showErrorNotification('error', 'Please fill all mandatory field');
                }
            });
        ...
        function handleDeleteFile(rowId, type, selectId = null, data = null) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $("#attachment_view-" + type + "-" + rowId).addClass("hidden").attr("src", response.file);
                    $.ajax({
        ...
        function handleGetClaimType(rowId, tableId, selectedId = null, data = null) {
            ...
            $.ajax({
                url: 'https://apidev.duluin.com/api/v1/payroll/expense_claim_type/datatables',
                method: 'POST',
                data: JSON.stringify({
        ...
        function getTableData() {
            ...
            $('#editable-expense-table tr').each(function() {
                let expense = {
                    ...
                    attachment_url: $(this).find('input[name="attachment"]').val()
                };
        ...
        function handleResponse(response) {
            if (response.success == true) {
                showSuccessNotification('success', 'Data has been created');
                setTimeout(() => {
