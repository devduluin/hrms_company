            ...
            <div class="mt-[65px] col-span-12 w-full">
                ...
                <div class="mt-3.5 grid grid-cols-12 gap-x-6 gap-y-10">
                    <div class="col-span-12 flex flex-col gap-y-7 sm:col-span-12 xl:col-span-12">
                        <form id="form-submit" method="post" action="{{ $apiUrl }}/expense_claim_type">
                            <div class="box box--stacked flex flex-col p-5">
                                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-5 mt-4">
                                    <div class="gap-x-6 gap-y-10 mb-5">
                                        <div>
                                            <x-form.input id="claim_type_name" label="Expense Claim Type"
                                                name="claim_type_name" value="{{ request()->get('item') }}" required />
                                        </div>
...
@push('js')
    ...
    <script type="text/javascript">
        ...
        async function handleGetData(type_id, currentForm) {
            path = `{{ $apiUrl }}/expense_claim_type/` + type_id;
            $.ajax({
                url: path,
                type: 'GET',
                headers: {
                ...
                function(response) {
                    if (response.success == true) {
                        method = 'PUT';
                        $("#claim_type_name").val(response.data.name);
                        $("#description").val(response.data.description);
                        $("select[name=company_id]").attr('data-selected', response.data.company_id).change();

                        const selectedValue = response.data.is_active;
        ...
        $("#form-submit").submit(async function(e) {
            ...
            let data = serializeFormData(currentForm);
            let formData = {
                ...data,
        ...
        if (type_id) {
            handleGetData(type_id, currentForm);
        }
