                            ...
                            <div class="col-span-12 flex flex-col gap-y-7 sm:col-span-12 xl:col-span-12">
                                <div class="box box--stacked flex flex-col p-5">
                                    ...
                                    <div class="absolute left-0 -mt-2 ml-4 bg-white px-3 text-xs uppercase text-slate-500">
                                        <div class="-mt-px">Employee Leave</div>
                                    </div>
                                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-5 mt-2 p-4">
                                            ...
                                            <div class="gap-x-6 gap-y-10 ">
                                                <div class="py-2">
                                                <x-form.select id="leave_type_id"  required name="leave_type_id" data-method="POST" label="Leave Type Name" url="{{ url('dashboard/hrms/employee/create') }}"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-5 p-4">
                                        ...
                                        <div>
                                            <x-form.textarea id="reason" label="Reason" name="reason" />   
                                        </div>
                                    </div>
                                    </div>
                                    ...
                                    <div class="relative mb-4 mt-4 rounded-[0.6rem] border border-slate-200/80 dark:border-darkmode-400 mr-5">
                                    <div class="absolute left-0 -mt-2 ml-4 bg-white px-3 text-xs uppercase text-slate-500">
                                        <div class="-mt-px">Approval</div>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-5 mt-2 p-4">
                                        
                                            <x-form.select id="approver_id" name="approver_id" data-method="POST" required label="Approver Name" url="{{ url('dashboard/hrms/employee/create') }}"
                                                apiUrl="{{ $apiUrlEmployee }}/datatables" columns='["first_name", "last_name"]'  
                                                :keys="[
                                                    'company_id' => $company_id,
                                                ]">
                                            ...
                                            <div class="mt-3 flex-row xl:items-center" placholder="">
                                                <div class="inline-block mb-2 sm:mb-0 sm:mr-5 sm:text-right xl:mr-4">
                                                    <div class="text-left">
                                                        <div class="flex items-center">
                                                            <div class="font-medium" for="start_time">Posting Date</div>
                                                                    <div class="ml-2.5 rounded-md border border-slate-200 bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-darkmode-300 dark:text-slate-400">
                                                                    Required
                                                                </div>                
                                                        </div>
                                                ...
                                                <div class="flex-1 sm:w-full  w-96  gap-1 mt-3 xl:mt-0">
                                                    <input id="posting_at" type="datetime-local" name="posting_at" data-single-mode="true" data-time="true" value=""  required="" class="disabled:bg-slate-100 disabled:cursor-not-allowed dark:disabled:bg-darkmode-800/50 dark:disabled:border-transparent [&amp;[readonly]]:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50 [&amp;[readonly]]:dark:border-transparent transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80 [&amp;[type='file']]:border file:mr-4 file:py-2 file:px-4 file:rounded-l-md file:border-0 file:border-r-[1px] file:border-slate-100/10 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-500/70 hover:file:bg-200 group-[.form-inline]:flex-1 group-[.input-group]:rounded-none group-[.input-group]:[&amp;:not(:first-child)]:border-l-transparent group-[.input-group]:first:rounded-l group-[.input-group]:last:rounded-r group-[.input-group]:z-10 ">
                                                </div>
                                            </div>
        ...
        $.ajax({
            ...
            function(response) {   
                if (response.success == true) {                    
                    ...
                    $("#posting_at").val(response.data.posting_at);
                    //$("select[name=status]").val(response.data.status).change();
                    addOptionIfNotExist('employee_id', response.data.employee_id);
                    addOptionIfNotExist('approver_id', response.data.approver_id);
                    addOptionIfNotExist('leave_type_id', response.data.leave_type_id);
