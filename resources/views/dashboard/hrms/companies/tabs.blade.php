...
<ul data-tw-merge role="tablist"
    ...
    @foreach (['overview' => 'building', 'preferences' => 'file'] as $id => $icon)
        <li id="{{ $id }}-tab" data-tw-merge role="presentation" class="focus-visible:outline-none flex-1">
            <button type="button" data-tw-merge data-tw-target="#{{ $id }}" role="tab"
                class="cursor-pointer block appearance-none px-3 py-2 border border-transparent text-slate-600 transition-colors dark:text-slate-400 rounded-md py-1.5 dark:border-transparent {{ $loop->first ? 'active' : '' }} [&.active]:text-primary [&.active]:dark:text-white [&.active]:border [&.active]:shadow-sm [&.active]:font-medium [&.active]:border-slate-200 [&.active]:bg-white [&.active]:dark:text-slate-300 [&.active]:dark:bg-darkmode-400 [&.active]:dark:border-darkmode-400 w-full">
