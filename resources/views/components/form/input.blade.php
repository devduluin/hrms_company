...
<div {{ $attributes->merge(['class' => $margin . ' flex-row xl:items-center'])->except(['id', 'label', 'name']) }}>
    ...
    @endif
    <div class="flex-1 sm:w-full @if ($type == 'number') w-40 @else sm:w-[90%] md:w-96 @endif gap-1 {{ $margin }} xl:mt-0">
        <input id="{{ $id }}" type="{{ $type }}" name="{{ $name }}" value="{{ $value }}"
            placeholder="{{ $label == null ? $placeholder : $label }}" @if ($readonly) readonly @endif @if ($required) required @endif
